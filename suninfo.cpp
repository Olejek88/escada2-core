...
int example_main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        struct tm lt = {0};
        localtime_r(&t, &lt);
        rise = rise + +lt.tm_gmtoff / 3600;
        unsigned int riseHour = rise;
        unsigned int riseMinute = 60 * (rise - riseHour);
        struct tm tmtm;
        ...
        switch (rs) {
            case 0:
                printf("Sun rises %02d:%02dh UT, sets %5.2fh UT\n",
                       riseHour, riseMinute, set + lt.tm_gmtoff / 3600);
                sprintf(finstr, "%04d-%02d-%02d %02d:%02d:%02d", year, month, day, riseHour, riseMinute, 0);
                printf("%s\n", finstr);
                //printf("unparse: %s\n", strptime(finstr, "%F %T", &tmtm));
                strptime(finstr, "%F %T", &tmtm);
                printf("tmtm ptr = %p\n", &tmtm);
                printf("tmtm.tm_hour=%d\n", tmtm.tm_hour);
                ttt = mktime(&tmtm);
                printf("%ld\n", ttt);
                break;
            case +1:
